...
const getThreshold = (color) => {
  if (color === "red") return 42;
    ...
    return 11; // For blue, we use the effective distance later.
...
const checkCollisionForColor = (
  ...
  opponentPieces.forEach((coin, idx) => {
    ...
    const coinCoord =
      coin.innerIndex !== null
        ? getInnerPath(opponentColor)[coin.innerIndex]
        : coin.position !== null
        ? indexToCoord[coin.position]
        : null;
    if (
      coinCoord &&
...
const Grid = () => {
    ...
    if (stepsRemaining === 0) {
      let finalCoord =
        currentInnerIdx !== null
          ? innerPath[currentInnerIdx]
          : indexToCoord[currentPos];
      if (
        currentInnerIdx !== null &&
        currentInnerIdx === innerPath.length - 1
      ) {
      ...
      if (eligibleCoins.length === 1) {
        ...
        if (coin.position === null && coin.innerIndex === null && dice === 6) {
          updateCoinPosition(
            currentTurn,
            coinIndex,
            getStartingPosition(currentTurn),
            null
          );
        } else {
          animateMovement(
  ...
  const cells = Array.from({ length: GRID_SIZE * GRID_SIZE }, (_, index) => {
    ...
    const isYellowInner = yellowInnerPath.some(
      (item) => item.row === row && item.col === col
    );
    if (isYellowInner && (row !== 5 || col !== 6)) {
      cellStyle.push({ backgroundColor: "#FFD700" });
    }
    const isRedInner = redInnerPath.some(
      (item) => item.row === row && item.col === col
    );
    if (isRedInner && (row !== 7 || col !== 6)) {
      cellStyle.push({ backgroundColor: "red" });
    }
    const isBlueInner = blueInnerPath.some(
      (item) => item.row === row && item.col === col
    );
    if (isBlueInner) {
      cellStyle.push({ backgroundColor: "#206ce6" });
    }
    const isGreenInner = greenInnerPath.some(
      (item) => item.row === row && item.col === col
    );
    if (isGreenInner) {
      cellStyle.push({ backgroundColor: "green" });
  ...
  return (
    <View style={styles.screen}>
      <Image
        source={require("../assets/ludofont.png")}
        style={styles.headerImage}
      />
      {/* Wrap game grid with a border showing current turn */}
      <View
        style={{ borderWidth: 30, borderColor: getBorderColor(currentTurn) }}
      >
        <View
          style={[
            styles.grid,
            { width: cellSize * GRID_SIZE, height: cellSize * GRID_SIZE },
          ]}
        >
          {cells}
          ...
          <View
            style={{
              ...
              top: (cellSize * 5 - innerBlockSize) / 2,
              left:
                cellSize * (GRID_SIZE - 5) +
                (cellSize * 5 - innerBlockSize) / 2,
              width: innerBlockSize,
              height: innerBlockSize,
              borderRadius: 15,
              borderWidth: 3,
          ...
          {redPieces.map((coin, idx) => {
            ...
            return (
              ...
              >
                <Text style={[styles.heartText, { fontSize: cellSize * 0.6 }]}>
                  ❤️
                </Text>
              </TouchableOpacity>
            );
      ...
      <TouchableOpacity onPress={() => navigation.navigate("GrabEgg")}>
        <Text style={styles.linkText}>Go to Grab Egg</Text>
      </TouchableOpacity>


      <TouchableOpacity onPress={() => navigation.navigate("TicTacToeMain")}>
        <Text style={styles.linkText}>Go to tic tac toe</Text>
      </TouchableOpacity>
    </View>
  );
};
